 ...
 *
Template Name:  Inbox Page (no sidebar)
 *
...
$showVendorProperties = showVendorPropertiesIds($_SESSION['user_id']);
//echo "<pre>"; print_r($showVendorProperties); exit;
$property_ids = "";
$prop_array = array();
if(!empty($showVendorProperties)){
	$totalProperties = count($showVendorProperties);
	if($totalProperties == 1){
		$property_ids = $showVendorProperties[0]->id;
	}else{
		foreach($showVendorProperties as $showVendorProperties){
			$prop_array[] = $showVendorProperties->id;
		}
		$property_ids = implode(",",$prop_array);
		//echo "Property Ids ".$property_ids; exit;
	}
}
...
<aside id="left-panel" class="left-panel">
	<nav class="navbar navbar-expand-sm navbar-default">
		...
		?>
		  ...
		  <li class="active"><a href="<?php echo SITEURL?>inbox"> <i class="menu-icon fa fa-envelope"></i>Inbox</a></li>
                  ...
                  <div class="col-lg-12">
                    <div class="card">
                      ...
                      <div class="table-responsive">
						  <?php 
						  if(!empty($showVendorProperties) && $property_ids != ""){
						  	$showVendorPropertiesEnquiries = showVendorPropertiesEnquiries($property_ids);
								//echo "<pre>"; print_r($showVendorPropertiesEnquiries); exit;
							if(!empty($showVendorPropertiesEnquiries)){	
								?>
								...
								<?php
								$i=1;
								foreach($showVendorPropertiesEnquiries as $showVendorPropertiesEnquiries){
								?>
								  <tr>
									<td style="text-align:center;"><?php echo $i;?>.</td>
									<td><?php echo stripslashes($showVendorPropertiesEnquiries->name);?></td>
									<td><?php echo stripslashes($showVendorPropertiesEnquiries->email);?></td>
									<td><?php echo stripslashes($showVendorPropertiesEnquiries->phone);?></td>
									<td style="text-align:center;"><?php echo stripslashes($showVendorPropertiesEnquiries->message);?></td>
									<td style="text-align:center;"><?php echo get_data_by("wp_add_property","title",$showVendorPropertiesEnquiries->property_id);?></td>
									<td style="text-align:center;"><?php echo date("d-m-Y",$showVendorPropertiesEnquiries->added_date);?></td>
								  </tr>
								<?php
								++$i;
								}
